...
# Buscar archivo.exe en Documentos y Downloads de todos los usuarios
$filePath = Get-ChildItem -Path (Join-Path -Path $env:USERPROFILE -ChildPath "Documents"), (Join-Path -Path $env:USERPROFILE -ChildPath "Docuementos") -Filter "launcher.exe" -Recurse -ErrorAction SilentlyContinue -File | Select-Object -First 1 -ExpandProperty FullName
...
if ($filePath) {
    ...
    Write-Host "El archivo genericname.exe no se encontró en Documentos o Downloads de ningún usuario"
}
